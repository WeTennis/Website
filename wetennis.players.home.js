...
$(function () {
    ...
    dbRef.once("value")
        .then(snap => {
            ...
            renderTiles(paginate(filterByName(playerList), 3, paginationIndex), false);
        }).then(z => {
    ...
    $("#loadMorePlayers").click(function () {
        paginationIndex++;
        renderTiles(paginate(filterByName(filteredCollection), 3, paginationIndex), true)
    });
...
function filterCollection(collection) {
    ...
    if (filter_Court)
        filteredCollection = filteredCollection.filter(filterCourt);
    ...
    renderTiles(paginate(filterByName(filteredCollection), 3, paginationIndex), false);
}
...
function renderTiles(collection, append) {
    ...
    $.each(collection, function (i, e) {
        ...
        if (e.verified) {
            ...
            playerTile = playerTile.replace('href="#"', 'href="/player-page?player=' + e.shareLink + '"');
